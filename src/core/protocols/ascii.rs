...
pub enum AsciiCommand {
    Get { keys: Vec<String> },
    Set { key: String, flags: u32, exptime: u32, bytes: usize, noreply: bool },
    Add { key: String, flags: u32, exptime: u32, bytes: usize, noreply: bool },
    Replace { key: String, flags: u32, exptime: u32, bytes: usize, noreply: bool },
    Delete { key: String, noreply: bool },
    Incr { key: String, value: u64, noreply: bool },
    Decr { key: String, value: u64, noreply: bool },
    FlushAll { exptime: Option<u32>, noreply: bool },
    Stats { args: Option<String> },
    Version,
    Quit,
}
...
pub enum AsciiResponse {
    Value { key: String, flags: u32, bytes: usize, data: Vec<u8> },
    Stored,
    NotStored,
    Exists,
    NotFound,
...
impl AsciiCommand {
    ...
    pub fn parse(line: &str) -> Result<Self, ProtocolError> {
        ...
        match command.as_str() {
            "GET" => {
                if parts.len() < 2 {
                    return Err(ProtocolError::ParseError("GET requires at least one key".to_string()));
                }
            ...
            "SET" | "ADD" | "REPLACE" => {
                if parts.len() < 5 {
                    return Err(ProtocolError::ParseError(format!("{} requires key, flags, exptime, bytes", command)));
                }
                let key = parts[1].to_string();
                let flags = parts[2].parse::<u32>().map_err(|_| ProtocolError::ParseError("Invalid flags".to_string()))?;
                let exptime = parts[3].parse::<u32>().map_err(|_| ProtocolError::ParseError("Invalid exptime".to_string()))?;
                let bytes = parts[4].parse::<usize>().map_err(|_| ProtocolError::ParseError("Invalid bytes".to_string()))?;
                let noreply = parts.len() > 5 && parts[5] == "noreply";

                match command.as_str() {
                    "SET" => Ok(AsciiCommand::Set { key, flags, exptime, bytes, noreply }),
                    "ADD" => Ok(AsciiCommand::Add { key, flags, exptime, bytes, noreply }),
                    "REPLACE" => Ok(AsciiCommand::Replace { key, flags, exptime, bytes, noreply }),
                    _ => unreachable!(),
                }
            }
            "DELETE" => {
            ...
            "INCR" | "DECR" => {
                if parts.len() < 3 {
                    return Err(ProtocolError::ParseError(format!("{} requires key and value", command)));
                }
                let key = parts[1].to_string();
                let value = parts[2].parse::<u64>().map_err(|_| ProtocolError::ParseError("Invalid value".to_string()))?;
                let noreply = parts.len() > 3 && parts[3] == "noreply";

                match command.as_str() {
                    "INCR" => Ok(AsciiCommand::Incr { key, value, noreply }),
                    "DECR" => Ok(AsciiCommand::Decr { key, value, noreply }),
                    _ => unreachable!(),
                }
            }
            "FLUSH_ALL" => {
                let exptime = if parts.len() > 1 && parts[1] != "noreply" {
                    Some(parts[1].parse::<u32>().map_err(|_| ProtocolError::ParseError("Invalid exptime".to_string()))?)
                } else {
                    None
                };
            ...
            _ => Err(ProtocolError::ParseError(format!("Unknown command: {}", command))),
        }
    }
...
impl AsciiResponse {
    ...
    pub fn parse(line: &str) -> Result<Self, ProtocolError> {
        ...
        if line.starts_with("VALUE ") {
            ...
            if parts.len() < 4 {
                return Err(ProtocolError::ParseError("Invalid VALUE response".to_string()));
            }
            let key = parts[1].to_string();
            let flags = parts[2].parse::<u32>().map_err(|_| ProtocolError::ParseError("Invalid flags".to_string()))?;
            let bytes = parts[3].parse::<usize>().map_err(|_| ProtocolError::ParseError("Invalid bytes".to_string()))?;
            Ok(AsciiResponse::Value { key, flags, bytes, data: Vec::new() })
        } else {
            match line {
                ...
                _ if line.starts_with("CLIENT_ERROR") => Ok(AsciiResponse::ClientError(line.to_string())),
                _ if line.starts_with("SERVER_ERROR") => Ok(AsciiResponse::ServerError(line.to_string())),
                _ if line.starts_with("STAT ") => {
                    let parts: Vec<&str> = line.splitn(3, ' ').collect();
                    if parts.len() >= 3 {
                        Ok(AsciiResponse::Stat { key: parts[1].to_string(), value: parts[2].to_string() })
                    } else {
                        Err(ProtocolError::ParseError("Invalid STAT response".to_string()))
                    }
                }
                ...
                _ => Err(ProtocolError::ParseError(format!("Unknown response: {}", line))),
            }
        }
    }
    ...
    pub fn format(&self) -> String {
        match self {
            AsciiResponse::Value { key, flags, bytes, .. } => {
                format!("VALUE {} {} {}\r\n", key, flags, bytes)
            }
...
impl AsciiProtocol {
    ...
    async fn parse_request(&self, client: &mut BufReader<TcpStream>) -> Result<(AsciiCommand, Vec<u8>), ProtocolError> {
        let mut line = String::new();
        client.read_line(&mut line).await
            .map_err(|e| ProtocolError::ParseError(format!("Failed to read command line: {}", e)))?;
        ...
        let data = match &command {
            ...
            AsciiCommand::Replace { bytes, .. } => {
                ...
                client.read_exact(&mut data).await
                    .map_err(|e| ProtocolError::ParseError(format!("Failed to read data block: {}", e)))?;

                // Read the trailing \r\n
                let mut trailing = [0u8; 2];
                client.read_exact(&mut trailing).await
                    .map_err(|e| ProtocolError::ParseError(format!("Failed to read trailing CRLF: {}", e)))?;
    ...
    async fn forward_request(&self, command: &AsciiCommand, data: &[u8], backend: &mut TcpStream) -> Result<Vec<u8>, ProtocolError> {
        // Reconstruct the original request
        let request = match command {
            AsciiCommand::Get { keys } => {
                format!("GET {}\r\n", keys.join(" "))
            }
            AsciiCommand::Set { key, flags, exptime, bytes, noreply } => {
                format!("SET {} {} {} {}{}\r\n",
                    key, flags, exptime, bytes,
                    if *noreply { " noreply" } else { "" })
            }
            AsciiCommand::Add { key, flags, exptime, bytes, noreply } => {
                format!("ADD {} {} {} {}{}\r\n",
                    key, flags, exptime, bytes,
                    if *noreply { " noreply" } else { "" })
            }
            AsciiCommand::Replace { key, flags, exptime, bytes, noreply } => {
                format!("REPLACE {} {} {} {}{}\r\n",
                    key, flags, exptime, bytes,
                    if *noreply { " noreply" } else { "" })
            }
            ...
            AsciiCommand::Incr { key, value, noreply } => {
                format!("INCR {} {}{}\r\n",
                    key, value, if *noreply { " noreply" } else { "" })
            }
            AsciiCommand::Decr { key, value, noreply } => {
                format!("DECR {} {}{}\r\n",
                    key, value, if *noreply { " noreply" } else { "" })
            }
            AsciiCommand::FlushAll { exptime, noreply } => {
                match exptime {
                    Some(exp) => format!("FLUSH_ALL {}{}\r\n",
                        exp, if *noreply { " noreply" } else { "" }),
                    None => format!("FLUSH_ALL{}\r\n",
                        if *noreply { " noreply" } else { "" }),
                }
            }
            AsciiCommand::Stats { args } => {
                match args {
                    Some(a) => format!("STATS {}\r\n", a),
                    None => "STATS\r\n".to_string(),
                }
        ...
        // Write the request
        backend.write_all(request.as_bytes()).await
            .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to write request: {}", e)))?;

        // Write data if present
        if !data.is_empty() {
            backend.write_all(data).await
                .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to write data: {}", e)))?;
            backend.write_all(b"\r\n").await
                .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to write trailing CRLF: {}", e)))?;
        }
    ...
    async fn read_response(&self, backend: &mut TcpStream) -> Result<Vec<u8>, ProtocolError> {
        ...
        loop {
            let mut line = String::new();
            let bytes_read = backend_reader.read_line(&mut line).await
                .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to read response line: {}", e)))?;

            if bytes_read == 0 {
                return Err(ProtocolError::ForwardingFailed("Backend closed connection".to_string()));
            }
            ...
            if line.starts_with("VALUE ") {
                ...
                if parts.len() >= 4 {
                    let bytes = parts[3].parse::<usize>()
                        .map_err(|_| ProtocolError::ParseError("Invalid bytes in VALUE response".to_string()))?;

                    // Read the data block
                    let mut data = vec![0u8; bytes];
                    backend_reader.read_exact(&mut data).await
                        .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to read value data: {}", e)))?;
                    response.extend(&data);

                    // Read the trailing \r\n
                    let mut trailing = [0u8; 2];
                    backend_reader.read_exact(&mut trailing).await
                        .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to read trailing CRLF: {}", e)))?;
            ...
            if line_trimmed == "END"
                ...
                || line_trimmed.parse::<u64>().is_ok()  // INCR/DECR response
            {
                break;
            }
        }
...
impl Protocol for AsciiProtocol {
    async fn handle_connection(&self, client: TcpStream, mut backend: TcpStream) -> Result<(), ProtocolError> {
        ...
        loop {
            ...
            if !response.is_empty() {
                ...
                client_stream.write_all(&response).await
                    .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to write response to client: {}", e)))?;
                client_stream.flush().await
                    .map_err(|e| ProtocolError::ForwardingFailed(format!("Failed to flush client response: {}", e)))?;
            }
        }
...
mod tests {
    ...
    fn test_get_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::Get { keys: vec!["mykey".to_string()] });

        // Multi-key GET
        let result = AsciiCommand::parse("GET key1 key2 key3\r\n").unwrap();
        assert_eq!(result, AsciiCommand::Get {
    ...
    fn test_set_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::Set {
            key: "mykey".to_string(),
            flags: 0,
            exptime: 300,
            bytes: 5,
    ...
    fn test_add_replace_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::Add {
        ...
        assert_eq!(result, AsciiCommand::Replace {
    ...
    fn test_delete_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::Delete {
            key: "mykey".to_string(),
            noreply: false
        });
    ...
    fn test_incr_decr_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::Incr {
        ...
        assert_eq!(result, AsciiCommand::Decr {
            key: "counter".to_string(),
            value: 3,
            noreply: true
        });
    ...
    fn test_flush_all_command_parsing() {
        ...
        assert_eq!(result, AsciiCommand::FlushAll {
            exptime: None,
            noreply: false
        });
    ...
    fn test_stats_version_quit_commands() {
        ...
        assert_eq!(result, AsciiCommand::Stats { args: Some("items".to_string()) });
    ...
    fn test_primary_key_extraction() {
        ...
        let cmd = AsciiCommand::Get { keys: vec!["key1".to_string(), "key2".to_string()] };
        assert_eq!(cmd.primary_key(), Some("key1"));

        // Storage commands
        let cmd = AsciiCommand::Set {
            key: "mykey".to_string(), flags: 0, exptime: 0, bytes: 5, noreply: false
        };
        assert_eq!(cmd.primary_key(), Some("mykey"));

        let cmd = AsciiCommand::Delete { key: "delkey".to_string(), noreply: false };
        ...
        let cmd = AsciiCommand::FlushAll { exptime: None, noreply: false };
        assert_eq!(cmd.primary_key(), None);
    }
    ...
    fn test_expects_response() {
        ...
        assert!(AsciiCommand::Get { keys: vec!["key".to_string()] }.expects_response());
        ...
        assert!(AsciiCommand::Set {
            key: "key".to_string(), flags: 0, exptime: 0, bytes: 5, noreply: false
        }.expects_response());

        // Commands with noreply=true
        assert!(!AsciiCommand::Set {
            key: "key".to_string(), flags: 0, exptime: 0, bytes: 5, noreply: true
        }.expects_response());

        assert!(!AsciiCommand::Delete {
            key: "key".to_string(), noreply: true
        }.expects_response());
    ...
    fn test_response_parsing() {
        ...
        assert_eq!(result, AsciiResponse::Value {
            key: "mykey".to_string(),
            flags: 123,
            bytes: 5,
            data: Vec::new()
        ...
        // Simple responses
        assert_eq!(AsciiResponse::parse("STORED").unwrap(), AsciiResponse::Stored);
        assert_eq!(AsciiResponse::parse("NOT_STORED").unwrap(), AsciiResponse::NotStored);
        assert_eq!(AsciiResponse::parse("EXISTS").unwrap(), AsciiResponse::Exists);
        assert_eq!(AsciiResponse::parse("NOT_FOUND").unwrap(), AsciiResponse::NotFound);
        assert_eq!(AsciiResponse::parse("DELETED").unwrap(), AsciiResponse::Deleted);
        assert_eq!(AsciiResponse::parse("TOUCHED").unwrap(), AsciiResponse::Touched);
        assert_eq!(AsciiResponse::parse("OK").unwrap(), AsciiResponse::Ok);
        assert_eq!(AsciiResponse::parse("END").unwrap(), AsciiResponse::End);

        // Error responses
        assert_eq!(AsciiResponse::parse("ERROR").unwrap(), AsciiResponse::Error("ERROR".to_string()));
        assert_eq!(AsciiResponse::parse("CLIENT_ERROR bad command").unwrap(),
                   AsciiResponse::ClientError("CLIENT_ERROR bad command".to_string()));
        assert_eq!(AsciiResponse::parse("SERVER_ERROR out of memory").unwrap(),
                   AsciiResponse::ServerError("SERVER_ERROR out of memory".to_string()));

        // STAT response
        let result = AsciiResponse::parse("STAT pid 12345").unwrap();
        assert_eq!(result, AsciiResponse::Stat {
            key: "pid".to_string(),
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(result, AsciiCommand::Get { keys: vec!["my:key-123_test".to_string()] });
