...
pub fn create_side_effect(
    config: &crate::config::SideEffectConfig,
) -> Result<Arc<dyn SideEffect>, SideEffectError> {
    match config.effect_type.as_str() {
        // New configurable replicator
        "replicate_on_hit" => {
            Ok(Arc::new(ReplicateOnHit::from_config(&config.config)) as Arc<dyn SideEffect>)
        }
        // Back-compat alias: defaults to first backend as target
        "replicate_to_primary_on_hit" => {
            Ok(Arc::new(ReplicateOnHit::default_primary()) as Arc<dyn SideEffect>)
        }
...
impl ReplicateOnHit {
    ...
    fn from_config(cfg: &std::collections::HashMap<String, Value>) -> Self {
        ...
        let mut target_indices: Vec<usize> = cfg
            ...
            .unwrap_or_default();
        if let Some(i) = single_index {
            target_indices.push(i);
        }
        let only_if_from_not_target = cfg
            .get("only_if_from_not_target")
    ...
    async fn on_concurrent_hit(&self, event: ConcurrentHitEvent) {
        ...
        for name in &self.target_backend_names {
                ...
                .find(|(_, b)| &b.name == name)
            {
                ...
                tracing::debug!(
                    "side_effect {}: target backend '{}' not found in pool; skipping",
                    self.name(),
                    name
                );
            }
        ...
        let Some((key, flags, bytes, data)) = Self::parse_value(&event.response_bytes) else {
            tracing::debug!(
                "side_effect {}: could not parse VALUE for replication",
                self.name()
            );
            return;
        };
        ...
        let verb = match self.method {
            ReplicationMethod::Set => "set",
            ReplicationMethod::Add => "add",
            ReplicationMethod::Replace => "replace",
        };
        ...
        tracing::debug!(
            "side_effect {}: replicating to {} target(s)",
            self.name(),
            targets.len()
        );
        for idx in targets {
            if let Some(target) = event.backends.get(idx) {
                ...
                tokio::spawn(async move {
                    ...
                    .await
                    {
                        Err(_) => {
                            tracing::debug!(
                                "side_effect replicate connect timeout to {} ({})",
                                name,
                                server
                            );
                        }
                        Ok(Ok(mut stream)) => {
                            tracing::debug!(
                                "side_effect {}: replicating key '{}' ({} bytes) to {}",
                                effect_name,
                                key_clone,
                                bytes,
                                name
                            ...
                            if let Err(e) = stream.write_all(&req).await {
                                tracing::debug!(
                                    "side_effect replicate write failed to {} ({}): {}",
                                    name,
                                    server,
                                    e
                                );
                                ...
                                tracing::debug!(
                                    "side_effect replicate flush failed to {} ({}): {}",
                                    name,
                                    server,
                                    e
                                );
                        ...
                        Ok(Err(e)) => {
                            tracing::debug!(
                                "side_effect replicate connect failed to {} ({}): {}",
                                name,
                                server,
                                e
                            );
